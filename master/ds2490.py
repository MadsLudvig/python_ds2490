...
from . import cstruct
from . import util
from . import GenericOneWireMaster
...
class DS2490Master(GenericOneWireMaster.GenericOneWireMaster):
  ...
  def __init__(self):
    ...
    self._device = usb.core.find(idProduct=self.PRODUCTID, idVendor=self.VENDORID)

    if not self._device:
      raise RuntimeError("no device")

    self._device.set_configuration()
    self._conf = self._device.get_active_configuration()
    self._intf = self._conf[(0,0)]
  ...
  def _SendMessage(self, command, value, index, timeout=TIMEOUT_LIBUSB):
    ret = self._device.ctrl_transfer(bmRequestType=0x40, bRequest=command,
                                  wValue=value, wIndex=index, timeout=timeout)
    if ret:
      raise UsbError(f"Error while sending control message: {ret}")
  ...
  @trace
  def GetStatus(self):
    raw = self._device.read(EP_STATUS, 32, TIMEOUT_LIBUSB)
    status = StatusPacket()
    status.UnpackFromTuple(raw[:16])
    result_regs = raw[16:]
    if result_regs:
      self._logger.info(f'result regs: {repr(result_regs)}')
  ...
  def RecvData(self, size):
    try:
      raw = self._device.read(EP_DATA_IN, size, TIMEOUT_LIBUSB)
    except usb.USBError:
  ...
  def SendData(self, buf):
    return self._device.write(EP_DATA_OUT, buf, TIMEOUT_LIBUSB)
  ...
  @trace
  def MultipleDeviceSearch(self, search_type=SEARCH_NORMAL, max=0):
    self.Reset()
  ...
  @trace
  def OneDeviceSearch(self, search_type=SEARCH_NORMAL, max=0):
    self.Reset()
    ...
    if raw:
      ...
      while len(ret) >= 8:
        ids = util.IdTupleToLong(ret[:8])
        ret = ret[8:]
        if len(str(hex(ids))) > 4:
          if str(hex(ids))[-4:] == "3601":
            return ids
